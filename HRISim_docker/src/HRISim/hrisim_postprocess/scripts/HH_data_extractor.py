        ...
        for task in tasks:
            if task.task_id not in self.tasks:
                self.tasks[task.task_id] = Task(task.task_id, task.start_time.to_sec(), task.path, task.final_destination,
                                                task.tot_inf_time, task.mean_inf_time, 0, task.evaluations)
                # self.tasks[task.task_id] = Task(task.task_id, task.start_time.to_sec(), task.path, task.final_destination,
                #                                 task.tot_inf_time, task.mean_inf_time, task.planning_time, task.evaluations)
...
def shutdown_callback(data_rows, filename, csv_path, data):   
    ...
    tasks = {id: {'start': task.starting, 
                  ...
                  'tot_query_inf_time': task.tot_query_inf_time} for id, task in data.tasks.items()}
