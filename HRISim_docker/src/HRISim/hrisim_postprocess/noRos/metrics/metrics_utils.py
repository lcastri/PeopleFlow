...
def readScenario(scenario):
    ...
    tree = ET.parse('/home/hrisim/ros_ws/src/HRISim/hrisim_postprocess/scenarios/' + scenario + '.xml')
    root = tree.getroot()
...
def compute_p_values(data, metric_name, alpha=0.05):
    base_data = np.array(data['base'][metric_name])
    versionA_data = np.array(data['versionA'][metric_name])
    versionB_data = np.array(data['versionB'][metric_name])
    ...
    if noncausal_normal and causal_normal:
        stat, p_value = ttest_ind(base_data, causal_data, equal_var=False)
        test_used = "T-Test"
    else:
        stat, p_value = mannwhitneyu(base_data, causal_data, alternative='two-sided')
...
def plot_boxplot(data, title, ylabel, bagnames, categories, background = False, outdir = None, fontsize=20):
    def flatten_mixed_data(data_for_bag):
        """Handles lists that contain both sublists and single floats."""
        flat_list = []
        if data_for_bag is None:
            return []
        for entry in data_for_bag:
            if isinstance(entry, (list, tuple, np.ndarray)):
                flat_list.extend(entry)
            else:
                try:
                    flat_list.append(float(entry))
    ...
    current_pos = 2 # Start at position 2, like your original plot

    for bag in bagnames:
        print("--------------------------------------------------")
        print(f"Processing bag: {bag} ({categories[bag]})")
        flat_data = flatten_mixed_data(data[bag][title])
        
        if not flat_data:
            print(f"Warning: No valid data for {categories[bag]}. Skipping.")
            continue
            
        try:
            # 2. Calculate stats. This returns a list, we take the first [0].
            # This is the magic: flat_data (gigabytes) becomes stats (kilobytes)
            stats = plt.cbook.boxplot_stats(flat_data)[0]
            
            # 3. Store the tiny stats dict
            all_stats.append(stats)
            
            # 4. Store the corresponding label and position
            all_labels.append(categories[bag])
            valid_positions.append(current_pos)
            
        except (IndexError, ValueError):
            # Failed to calculate stats (e.g., all NaNs)
            print(f"Warning: Could not calculate stats for {categories[bag]}. Skipping.")
            continue
            
        current_pos += 1
        # The giant flat_data list is now discarded, freeing memory

    if not all_stats:
        print(f"Error: No valid data could be processed for {title}.")
        plt.close()
        return

    # 4. Plot the stats using ax.bxp()
    ...
    # CRITICAL: bxp doesn't set labels, we must do it manually
    plt.xticks(valid_positions, all_labels, fontsize=fontsize)
    
    # Adjust xlim to match the *actual* number of plotted boxes
    plt.xlim(0.5, len(valid_positions) + 1.5) 
