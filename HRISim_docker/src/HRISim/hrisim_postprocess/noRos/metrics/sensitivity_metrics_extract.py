...
BAGNAME = ['KD1_KPD10_KBC5', 'KD1_KPD100_KBC50']
# BAGNAME = [
#     # KD = 0.1
...
for bag in BAGNAME:
    ...
    for tod in [TOD.H2, TOD.H6]:
        with open(os.path.join(INDIR, bag, f'tasks-{bag}-{tod.value}.json')) as pkl_file:
        ...
        for task_id in tqdm(TASK_IDs, desc=f"{bag}-{tod.value}"):
            ...
            TOD_METRICS[task_id]['evaluations'] = TASKS[str(task_id)]['evaluations']
            TOD_METRICS[task_id]['planning_time'] = TASKS[str(task_id)]['planning_time']
            TOD_METRICS[task_id]['mean_query_inf_time'] = TASKS[str(task_id)]['mean_query_inf_time']
            TOD_METRICS[task_id]['tot_query_inf_time'] = TASKS[str(task_id)]['tot_query_inf_time']
        ...
        TOD_METRICS['mean_evaluations'] = float(np.mean([TOD_METRICS[task]['evaluations'] for task in tmp_tasks]))
        TOD_METRICS['mean_planning_time'] = float(np.mean([TOD_METRICS[task]['planning_time'] for task in tmp_tasks]))
        TOD_METRICS['mean_query_inf_time'] = float(np.mean([TOD_METRICS[task]['mean_query_inf_time'] for task in tmp_tasks]))
        TOD_METRICS['tot_query_inf_time'] = float(np.sum([TOD_METRICS[task]['tot_query_inf_time'] for task in tmp_tasks]))
    ...
    METRICS['mean_evaluations'] = float(np.mean([METRICS[tod]['mean_evaluations'] for tod in METRICS.keys() if isinstance(METRICS[tod], dict)]))
    METRICS['mean_planning_time'] = float(np.mean([METRICS[tod]['mean_planning_time'] for tod in METRICS.keys() if isinstance(METRICS[tod], dict)]))
    METRICS['mean_query_inf_time'] = float(np.mean([METRICS[tod]['mean_query_inf_time'] for tod in METRICS.keys() if isinstance(METRICS[tod], dict)]))
    METRICS['tot_query_inf_time'] = float(np.sum([METRICS[tod]['tot_query_inf_time'] for tod in METRICS.keys() if isinstance(METRICS[tod], dict)]))
