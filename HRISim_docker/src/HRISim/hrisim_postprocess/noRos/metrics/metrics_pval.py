...
INDIR = '/home/hrisim/ros_ws/src/HRISim/hrisim_postprocess/csv/'
...
for bagname in BAGNAMES:
    ...
    pval_success_failure2.append([METRICS[tod.value][i]['result'] for tod in TOD2 for i in range(200)])
    pval_dangerous_interaction.append([METRICS['overall_human_collision']])
    pval_dangerous_interaction2.append([METRICS[tod.value][i]['human_collision'] for tod in TOD2 for i in range(200)])
    for tod in TOD2:
