...
def create_overlay_text():
    ...
    if TASKS is not None:
        ...
        tasks_detail_str = f"- Success: {TASKS[1]}/{int(rospy.get_param('/hrisim/tasks/total', 0))}\n- Failure: {TASKS[2]}/{int(rospy.get_param('/hrisim/tasks/total', 0))}" if TASKS is not None else 'none'
